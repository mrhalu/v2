  ...
  const addInventoriDarah = (sender, darah) => {
        const obj = {id: sender, healt: darah}
         _darahOrg.push(obj)
        fs.writeFileSync('./database/user/darah.json', JSON.stringify(_darahOrg))
   }
  const cekDarah = (sender) => {
            let status = false
  ...
  const addDarah = (sender, amount) => {
   ...
   module.exports = { addInventoriDarah, cekDarah, addDarah, kurangDarah, getDarah }   